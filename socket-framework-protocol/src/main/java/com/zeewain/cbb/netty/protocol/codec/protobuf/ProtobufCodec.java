package com.zeewain.cbb.netty.protocol.codec.protobuf;

import cn.hutool.core.util.TypeUtil;
import com.google.protobuf.Any;
import com.google.protobuf.Message;
import com.zeewain.cbb.netty.protocol.NettyProto;
import com.zeewain.cbb.netty.protocol.NettyResponse;
import com.zeewain.cbb.netty.protocol.codec.BodyCodec;
import com.zeewain.cbb.netty.protocol.util.InterfaceUtil;
import lombok.SneakyThrows;

import java.lang.reflect.Type;
import java.util.Optional;

import static com.zeewain.cbb.netty.protocol.codec.protobuf.ProtobufConvertUtil.*;
    ...
    public boolean support(Object msg) {
        ...
        if (msg instanceof NettyResponse) {
            ...
            if (resp.getData() == null) {
                ...
                return true;
            } else if (isProtobufType(getProtoType(resp.getData().getClass()))) {
                return true;
            }
        }

        return isProtobufType(getProtoType(msg.getClass()));
    }/*
    有两个小问题记录下；
    ...
    */
    ...
    public byte[] encode(Object msg) {
        ...
        if (msg instanceof NettyResponse) {
            ...
            if (data != null) {
                if (isProtobufInstance(data)) {
                    builder.setData(Any.pack((Message) data));
                } else if (isConverterInstance(data)) {
                    Class<Message> protoType = getProtoType(data.getClass());
                    builder.setData(Any.pack(convertToProtobufByCode((ProtobufConverter) data, protoType)));
                } else {
         ...
         */
        if (isConverterInstance(msg)) {
            Class<Message> protoType = getProtoType(msg.getClass());
            return convertToProtobufByCode((ProtobufConverter) msg, protoType).toByteArray();
        }
    ...
    public <T> T decode(byte[] bytes, Type type) {
        ...
        if (NettyProto.NettyResponse.class.isAssignableFrom(clazz)) {
            ...
            if (!protoResp.getData().equals(Any.getDefaultInstance())) {
                ...
                if (isProtobufType(firstTypeArg)) {
                    resp.setData(data);
                } else if (isConverterType(firstTypeArg)) {
                    resp.setData(convertFromProtobufByCode(data, firstTypeArg));
                } else {
        ...
        if (isConverterType(clazz)) {
            ...
            return (T) convertFromProtobufByCode(message, clazz);
        }
    ...
    private static Class<Message> getProtoType(Type type) {
        ...
        if (isProtobufType(actualClass)) {
            return (Class<Message>) actualClass;
        } else if (isConverterType(actualClass)) {
